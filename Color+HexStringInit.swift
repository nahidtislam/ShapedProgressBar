...
extension Color {
    init?(hex: String, colorSpace: RGBColorSpace = .sRGB) {
        ...
        guard input.first == "#" && [7, 9].contains(input.count) else { return nil }
        ...
        if input.count == 7 {
            ...
            let aHex = hexi(section: 4)
            let aValue = floatThe(string: aHex)
            
            self.init(colorSpace, red: rValue, green: gValue, blue: bValue, opacity: aValue)
        }
    }
